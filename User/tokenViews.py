...
from  StudentManager.settings import SECRET_KEY
import jwt
import json
from datetime import datetime,timedelta,timezone
from rest_framework.decorators import APIView
from Role.roleModels import Role
from UserRole.userRoleModels import UserRole
...
class TokenApi(APIView):
    ...
    def post(self,request):   
        ...
        try:
            sql="Select * from  user_user where user_user.Username ='"+ userRequestToken['Username']+ "' and user_user.Password = '"+userRequestToken['Password']+"'"
...
#---------Code Phòng chóng tấn công bằng sql injection
# class TokenApi(APIView):
#     @csrf_exempt
#     def post(self,request):
#         exp=datetime.now(tz=timezone.utc) + timedelta(minutes=50)
